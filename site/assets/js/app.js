...
(function () {
  ...
  var CONFIG = {
    ...
    /* Оплата на сайте.
     *
     * Сайт — статика на GitHub Pages, оплату считает Cloudflare Worker
     * из папки worker/. Адрес полный, а не относительный: домен
     * обслуживают DNS-серверы reg.ru, а маршрут вида hollvpn.ru/api/*
     * бывает только у доменов внутри Cloudflare. Так что запрос идёт
     * на служебный адрес Worker'а напрямую.
     *
     * Это межсайтовый запрос, и он проходит только потому, что Worker
     * пускает ровно один origin — свой PUBLIC_URL, https://hollvpn.ru.
     * Если сайт переедет на другой адрес, поменяйте PUBLIC_URL там же,
     * иначе браузер запрос не отдаст.
     *
     * Пока Worker не развёрнут, страница оплаты честно отправляет
     * в бота, а не показывает ошибку. Пустая строка в checkout убирает
     * ссылку «оплатить на сайте» — на случай, если оплату на сайте надо
     * быстро выключить. */
    checkout: '/checkout.html',
    checkoutApi: 'https://tg-bot.neverdeadsd.workers.dev/api',
