...
(function () {
  ...
  var CONFIG = {
    ...
    /* Оплата на сайте.
     *
     * Сайт — статика на GitHub Pages, а /api/ отвечает Cloudflare Worker
     * из папки worker/ на том же домене: поэтому путь относительный
     * и CORS не нужен. Пока Worker не развёрнут, страница оплаты честно
     * отправляет в бота, а не показывает ошибку.
     *
     * Пустая строка в checkout полностью убирает ссылку «оплатить
     * на сайте» — на случай, если оплату на сайте надо быстро выключить.
     * Если API уедет на другой домен, впишите сюда полный адрес и добавьте
     * origin сайта в PUBLIC_URL Worker'а, иначе CORS его не пропустит. */
