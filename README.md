...
Бот считает, из какого канала пришёл клиент. Метка передаётся прямо в ссылке:

```
https://t.me/brief7_bot?start=avito
https://t.me/brief7_bot?start=kwork
https://t.me/brief7_bot?start=partner_ivanov
```

Разрешены латиница, цифры, `-` и `_`, до 32 символов; всё остальное отбрасывается,
и заход считается прямым. Метка привязывается к пользователю **по первому касанию** —
если он потом придёт по другой ссылке, источник не перепишется, иначе последний канал
присваивал бы себе чужого клиента.

Где источник видно:

- в карточке заявки админу — второй строкой, рядом с датой;
- в `/order <номер>`;
- в `/stats` — разбивка по каналам;
- в выгрузке `/export` — отдельная колонка `source`.

Схема базы обновляется сама: в существующую БД колонка добавляется при старте,
заявки, собранные раньше, остаются с пустым источником.
