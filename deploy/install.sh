...
# Панели VPN (Hiddify, Marzban, 3x-ui, Remnawave) держат 80 и 443 своим
# haproxy или nginx и перегенерируют конфигурацию при каждом «применить».
# Ставить сюда сайт нельзя по двум причинам, и вторая важнее первой.
for panel in /opt/hiddify-manager /opt/hiddify-config /opt/marzban \
             /usr/local/x-ui /opt/remnawave /opt/marzneshin; do
  [ -e "$panel" ] || continue
  die "На сервере стоит панель VPN: $panel

  Она занимает порты 80 и 443 и переписывает свои конфигурации сама —
  установить рядом сайт не выйдет, а правки руками проживут до первого
  «применить» в панели.

  Но главное другое: продающий сайт и рабочая нода не должны делить IP.
  Заблокируют домен — под блокировку попадёт адрес, а вместе с ним нода,
  через которую ходят платящие клиенты. И наоборот.

  Ставьте сайт на отдельный сервер или выкладывайте статику из site/
  на GitHub Pages — она для этого и написана без зависимостей."
done
